...
public final class GroundMoveSpeedModifierSystem extends EntityTickingSystem<EntityStore> {
    ...
    private final LazyStatIndex moveSpeedIndex = new LazyStatIndex(ArmoryStatIds.MOVE_SPEED);
    ...
    public void tick(
        ...
        if (Math.abs(live - target) <= StatMultiplier.EPSILON) {
            return;
        }
    ...
    private float getMoveSpeedMultiplier(@Nonnull EntityStatMap statMap) {
        return StatMultiplier.forStat(statMap, moveSpeedIndex.get());
    }
}
